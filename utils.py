...
def create_parser():
    ...
    parser.add_argument("--group_name", type=str, default="test",
                        help="Name of the wandb group of experiment")
    parser.add_argument("--project_name", type=str, default="crl",
                        help="Name of the wandb project of experiment")
    parser.add_argument("--num_timesteps", type=int,
                        default=1000000, help="Number of training timesteps")
    parser.add_argument("--max_replay_size", type=int,
                        default=10000, help="Maximum size of replay buffer")
    parser.add_argument("--min_replay_size", type=int,
                        default=8192, help="Minimum size of replay buffer")
    parser.add_argument("--num_evals", type=int, default=50,
                        help="Total number of evaluations")
    parser.add_argument("--episode_length", type=int,
                        default=50, help="Maximum length of each episode")
    parser.add_argument("--action_repeat", type=int, default=2,
                        help="Number of times to repeat each action")
    parser.add_argument("--discounting", type=float,
                        default=0.997, help="Discounting factor for rewards")
    ...
    parser.add_argument("--unroll_length", type=int,
                        default=50, help="Length of the env unroll")
    parser.add_argument("--train_step_multiplier", type=int, default=1,
    ...
    parser.add_argument("--log_wandb", default=False,
                        action="store_true", help="Whether to log to wandb")
    ...
    parser.add_argument('--contrastive_loss_fn', type=str,
                        default='symmetric_infonce', help="Name of the contrastive loss function")
    parser.add_argument('--energy_fn', type=str, default='l2',
                        help="Function to calculate energy")
    parser.add_argument('--backend', type=str, default=None,
                        help="Backend to be used for the environment")
    parser.add_argument('--no_resubs', default=False, action='store_true',
                        help="Not use resubstitution (diagonal) for logsumexp in contrastive cross entropy")
    parser.add_argument('--use_ln', default=False, action='store_true',
                        help="Whether to use layer normalization for preactivations in hidden layers")
    parser.add_argument('--logsumexp_penalty', type=float, default=0.0,
                        help="Penalty for logsumexp in contrastive loss")
    parser.add_argument('--l2_penalty', type=float,
                        default=0.0, help="L2 penalty for regularization")
    parser.add_argument('--random_goals', type=float, default=0.0,
                        help="Proportion of random goals to use in the actor loss")
    parser.add_argument('--disable_entropy_actor', default=False,
                        action="store_true", help="Whether to disable entropy in actor")
    parser.add_argument('--eval_env', type=str, default=None,
                        help="Whether to use separate environment for evaluation")
    ...
    parser.add_argument('--use_dense_reward', default=False,
                        action="store_true", help="Whether to use sparse reward in env")
    parser.add_argument('--use_her', default=False,
                        action="store_true", help="Whether to use HER for SAC")
    parser.add_argument('--visualization_interval', type=int, default=5,
                        help="Number of evals between each visualization of trajectories")
    parser.add_argument('--dual_lam', type=float, default=0.5,
                        help="Dual variable for safety constraint")
    return parser
...
def create_env(env_name: str, backend: str = None, **kwargs) -> object:
    ...
    if env_name == "reacher":
        ...
        if "ant_ball" in env_name:
            env = AntBallMaze(backend=backend or "spring",
            ...
            env = HumanoidMaze(backend=backend or "spring",
                               maze_layout_name=env_name[9:])
        else:
...
class MetricsRecorder:
    ...
    def print_progress(self):
        for idx, (key, y_values) in enumerate(self.y_data.items()):
            print(f"step: {
                  self.x_data[-1]}, {key}: {y_values[-1]:.3f} +/- {self.y_data_err[key][-1]:.3f}")
    ...
    def progress(self, num_steps, metrics, make_policy, params, env, do_render=True):
        ...
        self.record(num_steps, {key: value for key, value in metrics.items(
...
def render(make_policy, params, env, exp_dir, exp_name, num_steps):
    ...
    url = html.render(env.sys.tree_replace(
        {"opt.timestep": env.dt}), rollout, height=1024)
